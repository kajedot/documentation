...
Alternatively, you can define any :ref:`static <pages>` page as your homepage by going to
:menuselection:`Website --> Site --> Properties`. Select the :guilabel:`Publish` tab and enable
...
To modify a :ref:`static <pages>` page's properties, access the page you wish to modify, then go to
...
move your site to a new :doc:`domain <configuration/domain_names>`. It can also be used to
...
.. note::
   ...
   - You can set up redirections for :ref:`static and dynamic pages <pages>`.
...
- :guilabel:`Action`: Select the type of redirection:
   ...
   - :guilabel:`301 Moved Permanently`: for permanent redirections of unpublished or deleted
     *static* pages. The new URL is shown in search engine results, and the redirect is cached by
     browsers.
   ...
   - :guilabel:`308 Redirect/Rewrite`: for permanent redirections of existing :ref:`dynamic <pages>`
     pages. The URL is renamed. The new name is shown in search engine results and is cached by
     browsers. Use this redirect type to rename a dynamic page, for example, if you wish to rename
     `/shop` into `/market`. The 308 redirect rewrites the URL link but does not show any new
     view/resource.

- :guilabel:`URL from`: Enter the URL to be redirected (e.g., `/about-the-company`) or search for
  the desired :ref:`dynamic <pages>` page and select it from the list.
