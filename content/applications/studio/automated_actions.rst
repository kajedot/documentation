...
#. Select the :ref:`studio/automated-actions/trigger` and, depending on the trigger you chose, fill
...
.. tip::
   ...
   - You can also create automation rules from any kanban stage by clicking the gear icon
     ...
     default, the :guilabel:`Stage is set to` trigger is selected in this case, but you can change it
     if necessary.
...
.. tip::
   For some triggers, you can define a :guilabel:`Before Update Domain` to specify the conditions
   that must be met *before* the automation rule is triggered. In contrast, the conditions defined
   using the :ref:`Extra Conditions <studio/automated-actions/trigger/values-timing-conditions>`
   and :ref:`Apply on <studio/automated-actions/trigger/custom>` filters are checked *during* the
...
- a :guilabel:`Delay`: Specify the number of minutes, hours, days, or months. To trigger the action
  ...
  field` trigger, you must also select the field to be used to determine the delay.
...
To configure the :guilabel:`Webhook` trigger, copy the :guilabel:`URL` generated by Odoo into the
...
.. warning::
   The URL must be treated as confidential; sharing it online or without caution could potentially
   expose your system to malicious parties.
...
To do so, select or create an :guilabel:`Email Template` or an an :guilabel:`SMS Template`, then, in
...
Use these actions to (un)subscribe existing contacts to the record.
...
Specify a :guilabel:`Name` for the record, then, optionally, select a field in the
:guilabel:`Link Field` field to link the record that triggered the creation of the new record.
